...
public class Triangle : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        MakeVerticesAndSetScaleAndToArray();
        MakeIndeciesToMakeRectangle();

        SetVerticesToMeshFilter(verticesArray);
    ...
    void MakeVerticesAndSetScaleAndToArray()
    {
        for (int z = 0; z < totalVerticesOnZ; z++)
        {
            for (int x = 0; x < totalVerticesOnX; x++)
            {
                ...
                verticesArray[index] = new Vector3(x, 0, z);
                ScaleVertexBasedOnTextureImagePixelColor(x, z, ref verticesArray[index]);
            }
        }
    }
